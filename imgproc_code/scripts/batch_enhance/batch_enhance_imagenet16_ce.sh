...
for eps in "${custom_eps[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=${GPU_ID}, python enhance.py --eps $eps --num_steps $num_steps --dest_dir data/${DATASET_NAME} --dirmap_path ${DIRMAP_CSV_PATH} --dataset_name ${DATASET_NAME_RB} --dataset_path ${DATASET_PATH} --model_ckpt_path ${MODEL_PATH} --objective_type cross_entropy ${save_orig}

    if $DO_ATTACK; then
        CUDA_VISIBLE_DEVICES=${GPU_ID}, python enhance.py --eps $eps --num_steps $num_steps --dest_dir data/${DATASET_NAME} --dirmap_path ${DIRMAP_CSV_PATH} --dataset_name ${DATASET_NAME_RB} --dataset_path ${DATASET_PATH} --model_ckpt_path ${MODEL_PATH} --objective_type cross_entropy --attack
    fi
