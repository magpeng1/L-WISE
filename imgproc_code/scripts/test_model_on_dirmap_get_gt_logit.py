...
def get_args(argv):
    ...
    parser.add_argument('--suffix', type=str, default="", help="Something to add to the end of the file name")
...
def test_model_accuracy_and_generate_confusion_matrix(**kwargs):
    ...
    dataset = DirmapDataset(csv_file=kwargs['dirmap_path'], transform=transform, class_num_col=kwargs['class_num_col'], dataset_path=kwargs["dataset_path"], split=kwargs['split'])
    ...
    logit_col_name = 'vanilla_gt_logit' if kwargs['vanilla'] else 'robust_gt_logit'
    ...
    if len(suffix) > 0 and suffix[0] != "_":
        suffix = "_" + suffix
    csv_save_path = base_csv_path + "_logits" + suffix + ext
