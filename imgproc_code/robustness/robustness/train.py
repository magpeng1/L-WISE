...
def make_optimizer_and_schedule(args, model, checkpoint, params):
    ...
    *Internal Function* (called directly from train_model)
    ...
    if args.custom_lr_multiplier == 'cyclic':
        ...
        def cosine_annealing_with_warmup(t, T, lr_max, warmup_epochs):
            if t < warmup_epochs:
                return lr_max * (t / warmup_epochs)
            else:
                return lr_max * (1 + np.cos(np.pi * (t - warmup_epochs) / (T - warmup_epochs))) / 2
        warmup_epochs = 5
        lr_func = lambda t: cosine_annealing_with_warmup(t, args.epochs, args.lr, warmup_epochs)
        schedule = lr_scheduler.LambdaLR(optimizer, lr_func)
...
def train_model(args, model, loaders, *, checkpoint=None, dp_device_ids=None,
            store=None, update_params=None, disable_no_grad=False, verbose=False):
    """
    ...
    if checkpoint:
        ...
        best_prec1 = checkpoint[prec1_key] if prec1_key in checkpoint \
            else _model_loop(args, 'val', val_loader, model, None, start_epoch-1, args.adv_train, writer=None, verbose=verbose)[0]
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_prec1, train_loss = _model_loop(args, 'train', train_loader, 
                model, opt, epoch, args.adv_train, writer, verbose=verbose)
        ...
        if should_log or last_epoch or should_save_ckpt:
            ...
            with ctx:
                prec1, nat_loss = _model_loop(args, 'val', val_loader, model, 
                        None, epoch, False, writer, verbose=verbose)
            ...
            adv_val = should_adv_eval and _model_loop(args, 'val', val_loader,
                    model, None, epoch, True, writer, verbose=verbose)
        ...
        # Clear cache at the end of each epoch (Morgan T. added)
...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer, verbose=False):
    """
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        if loop_type == 'train' and has_attr(args, 'mixup_fn'):
            if len(inp) % 2 != 0:
                print("WARNING: encountered oddly-sized batch. Removing final example to make it evenly-sized for cutmix/mixup.")
                inp = inp[:-1]  # Remove last example if odd-sized batch (needs to be even for cutmix/mixup augmentation)
                target = target[:-1]
            inp, target = args.mixup_fn(inp, target)
            target = target.to(ch.float)

        # Zero gradients before forward pass (Morgan T. added)
        if is_train:
            opt.zero_grad()
        ...
        try:
            ...
            if has_attr(args, "custom_accuracy"):
                ...
                if loop_type == 'train' and has_attr(args, 'mixup_fn'):
                    # Soft accuracy for mixed data (cutmix/mixup)
                    with ch.no_grad():
                        acc = (output.argmax(1) == target.argmax(1)).float().mean()
                        prec1 = acc * 100.0
                        prec5 = float('nan')  # top-5 doesn't make sense for soft labels
                else:
                    prec1, prec5 = helpers.accuracy(model_logits, target, topk=(1, maxk))

            losses.update(loss.item(), inp.size(0))
            top1.update(to_item(prec1), inp.size(0))
            top5.update(to_item(prec5), inp.size(0))
        ...
        # Clear unnecessary tensors (Morgan T. added)
        del output, final_inp, loss
        if i > 0 and i % 10 == 0 and ch.cuda.is_available(): # Clear cache every 10 epochs
            ch.cuda.empty_cache()
    ...
    # Reset model to eval mode and clear cache (Morgan T. added)
    model.eval()
    if ch.cuda.is_available():
        ch.cuda.empty_cache()
