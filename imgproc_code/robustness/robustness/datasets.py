...
class DataSet(object):
    ...
    def make_loaders(self, workers, batch_size, data_aug=True, subset=None, 
                    ...
                    subset_seed=None, transforms=None):
        '''
        Args:
...
class MHIST(DataSet):
    """
    Classes are hp (hyperplastic polyp) and ssa (sessile serrated adenoma)
    """
    def __init__(self, data_path='/media/KLAB37/morgan_data/mhist/robustness', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 2,
            'mean': ch.tensor([0.73781023, 0.64857995, 0.77524428]), # Updated for MHIST
            'std': ch.tensor([0.19724766, 0.24365536, 0.17030507]),
            'custom_class': None,
            'label_mapping': None,
            'transform_train': da.TRAIN_TRANSFORMS_HIST,
            'transform_test': da.TEST_TRANSFORMS_TOTENSOR
        }
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
        super(MHIST, self).__init__('mhist', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('WBC dataset does not support pytorch_pretrained=True')
        return imagenet_models.__dict__[arch](num_classes=self.num_classes)
...
class inat(DataSet):
    ...
    """
    def __init__(self, data_path='/n/lw_groups/bch/ophth/kreiman/lab/morgan_datasets/inat2021', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 10000,
        ...
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
        super(inat, self).__init__('inat', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('inat dataset does not support pytorch_pretrained=True')
        return imagenet_models.__dict__[arch](num_classes=self.num_classes)
    

class idaea4(DataSet):
    """
    idaea4 dataset (drawn from 4 species of moths in inat (the idaea genus))
    """
    def __init__(self, data_path='/home/mbt10/projects/learn-histopath-backend/data/idaea4_all', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 4,
        ...
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
        super(idaea4, self).__init__('idaea4', data_path, **ds_kwargs)

    def get_model(self, arch, pretrained):
        """
        """
        if pretrained:
            raise ValueError('idaea4 dataset does not support pytorch_pretrained=True')
        return imagenet_models.__dict__[arch](num_classes=self.num_classes)
...
class HAM10000(DataSet):
    ...
    HAM10000 Dataset
    """
    def __init__(self, data_path='/n/lw_groups/bch/ophth/kreiman/lab/morgan_datasets/HAM10000/robustness', **kwargs):
        """
        """
        ds_kwargs = {
            'num_classes': 7,
            'mean': ch.tensor([0.76498297, 0.57012301, 0.59907285]), # This is from 256 random images per class after oversampling to match majority class
            'std': ch.tensor([0.12553158, 0.14498346, 0.15726216]),
            'custom_class': None,
            'label_mapping': None,
            'transform_train': da.TRAIN_TRANSFORMS_HAM10000,
            'transform_test': da.TEST_TRANSFORMS_HAM10000
        }
        ds_kwargs = self.override_args(ds_kwargs, kwargs)
