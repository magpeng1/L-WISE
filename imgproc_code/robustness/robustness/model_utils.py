...
def make_and_restore_model(*_, arch, dataset, resume_path=None,
         parallel=False, pytorch_pretrained=False, add_custom_forward=False, gpu_id=None):
    """
    ...
    if resume_path and os.path.isfile(resume_path):
        ...
        try:
            ...
            checkpoint = ch.load(resume_path, pickle_module=dill, map_location="cuda:0")

        if isinstance(checkpoint, transformers.ViTModel):
            model.model.model = checkpoint # We need model.model.model because it goes AttackerModel.VitWrapper.model
            print("=> loaded model directly from '{}'".format(resume_path))
        else:
            ...
            if state_dict_path in checkpoint:
                sd = checkpoint[state_dict_path] # Morgan indented
                sd = {k[len('module.'):]:v for k,v in sd.items()}
            else: # Added for compatibility with checkpoints that have all the model layers at the top level (i.e., no "state_dict" entry)
                sd = checkpoint
