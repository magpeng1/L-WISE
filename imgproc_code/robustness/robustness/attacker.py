...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
                ...
                est_grad=None, mixed_precision=False, get_all_step_ims=False):
        """
        ...
        step = step_class(eps=eps, orig_input=orig_input, step_size=step_size, model=self)
        ...
        def get_adv_examples(x, get_all_step_ims=False):

            if get_all_step_ims:
                all_step_ims = [x.clone().detach().cpu()]
            ...
            for _ in iterator:
                ...
                with ch.no_grad():
                    ...
                    if get_all_step_ims:
                        all_step_ims.append(step.to_image(x.clone().detach().cpu()))
            ...
            if not use_best: 
                ...
                if get_all_step_ims:
                    if return_image:
                        return step.to_image(ret), all_step_ims
                    else:
                        return ret, all_step_ims
                else:
            ...
            if get_all_step_ims:
                if return_image:
                    return step.to_image(best_x), all_step_ims
                else:
                    return best_x, all_step_ims            
            else:
        ...
        if random_restarts:
            ...
            for _ in range(random_restarts):
                if get_all_step_ims:
                    adv, all_step_ims = get_adv_examples(orig_cpy, get_all_step_ims=get_all_step_ims)
                else:
            ...
            if get_all_step_ims:
                adv_ret, all_step_ims = get_adv_examples(x, get_all_step_ims=True)
            else:
                adv_ret = get_adv_examples(x)

        if get_all_step_ims:
            return adv_ret, all_step_ims
        else:
            return adv_ret
...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
                fake_relu=False, no_relu=False, with_image=True, get_all_step_ims=False, **attacker_kwargs):
        """
        ...
        if make_adv:
            ...
            self.eval()
            if get_all_step_ims:
                adv, all_step_ims = self.attacker(inp, target, get_all_step_ims=get_all_step_ims, **attacker_kwargs)
            else:
        ...
        if with_latent or fake_relu or no_relu: # Morgan added this for compatibility with models not trained using the Robustness library
            output = self.model(normalized_inp, with_latent=with_latent, fake_relu=fake_relu, no_relu=no_relu)
        else:
            output = self.model(normalized_inp)

        if with_image:
            if get_all_step_ims:
                return (output, inp, all_step_ims)
            else:
                return (output, inp)
        else:
            if get_all_step_ims:
